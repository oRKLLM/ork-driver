...
static const ork_reg_desc ORK_REGS[RK_REG__COUNT] = {
    ...
    [RK_CNA_CBUF_CON0]        = {0x201, 0x1040, 0x00003fff,  "CNA_CBUF_CON0",        "DATA_BANK[3:0] WEIGHT_BANK[7:4] FC_DATA_BANK[10:8] DATA_REUSE[12] WEIGHT_REUSE[13] (rocket_registers.h). rkllm leaves REUSE=0. #39 found reuse-tile output WRONG -- but that was the FOLD path only, whose 0x1040/0x1030 are baked from a K=3584 N=1216 capture (4.36 MB weight, K/N unpatchable). On the COLSPLIT M-tile path, where 0x1040 is synthesised from mc, WEIGHT_REUSE is BIT-EXACT and up to 1.52x -- but ONLY at K=4096/Sn==1 (ORK_MTILE_WR=1, gated to that measured envelope). Applied wider it fails make test at K=3584/K=2048/Sn=3, and K=3584 has the SAME mcap as K=4096, so the boundary is not M-tile geometry and is UNKNOWN"},
