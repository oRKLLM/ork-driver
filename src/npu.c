...
struct ork_npu { int fd; const struct ork_soc *soc; struct buf regcmd, task, Af, Cc; size_t ccsz; void *cres; size_t cressz; int warmed, last_dt; int core_budget;
    ...
    int ssm_nc, ssm_nr, ssm_nh; ork_w **ssm_pS,**ssm_pD,**ssm_pC,**ssm_pO;
...
static int run_multicore(ork_npu *c,ork_w *w,int M,const void *A,void *C,int nc){
    ...
    if(nc<1) nc=1;
    if(dt!=c->last_dt){ int keepwarm=(ork_nothrash()&&ORK_INT_DT(dt)&&ORK_INT_DT(c->last_dt)) || (ork_f16warm()&&ORK_KW_DT(dt)&&ORK_KW_DT(c->last_dt)); if(dt==DT_I8 && !ORK_I8_LIVE(c->last_dt) && !keepwarm) act(fd,RKNPU_ACT_RESET,0); for(int i=0;i<ORK_MAXCORE;i++){ if(!keepwarm)c->mwarm[i]=0; if(!ork_nothrash()&&!keepwarm)c->mccsz[i]=0; } c->last_dt=dt; }
...
static int run(ork_npu *c,ork_w *w,int M,const void *A,void *C){
    ...
    if(dt!=c->last_dt){ int kw=ork_f16warm()&&ORK_KW_DT(dt)&&ORK_KW_DT(c->last_dt); if(dt==DT_I8 && !ORK_I8_LIVE(c->last_dt) && !kw) act(fd,RKNPU_ACT_RESET,0); if(!kw)c->warmed=0; if(!ork_nothrash()&&!kw)c->ccsz=0; c->last_dt=dt; }
...
int ork_mm_run_f16_silu(ork_npu *c,ork_w *w,int M,const ork_f16 *A,float *C,
    ...
    if(w->domain!=c->dom_active || (w->domain!=0 && !c->dom_save)) dom_activate(c,w->domain);
    if(DT_F16!=c->last_dt){ int kw=ork_f16warm()&&ORK_KW_DT(c->last_dt); if(!kw)c->warmed=0; if(!ork_nothrash()&&!kw)c->ccsz=0; c->last_dt=DT_F16; }   /* NOTHRASH: reuse Cc, no realloc under IOVA pressure (see run()) */
 ...
 * act_lut_i8/i16(fn,...) path). Bakes an arbitrary scalar fn(x) into the SDP output-stage LUT for inputs
...
static double f16lut_silu(double x, void *ctx){ (void)ctx; return silu_f(x); }
 ...
 * tensor is ork_npu_rsqrt_i8/i16 (built via act_lut_i8/i16 with the same rsqrt_f). Same op, different
 ...
 * DPU_RDMA (0x5038) — set_i8_ewmul + splice_ew_lane graft it onto ork's conv+int8-out program (regcfg
...
int ork_mm_run_i8_ewmul(ork_npu *c,ork_w *w,int M,const int8_t *A,const int8_t *G,int8_t *C,int mult,int shift){
    ...
    struct buf Gb=bcreate(fd,gsz,0x403,c->dom_active); if(!Gb.cpu)return -2;
    int rc_ret=0;
    for(int ns=0;ns<w->Sn && rc_ret==0;ns++){ int n0=ns*NMAX,Nc=(N-n0<NMAX)?(N-n0):NMAX;
        uint64_t wbase=w->Bf[ns].dma;
        for(int m0=0;m0<M && rc_ret==0;m0+=chunk){ int mc=(M-m0<chunk)?(M-m0):chunk; if(mc<=0)continue;
            ...
            { struct rknpu_task *t=c->task.cpu; memset(t,0,sizeof *t);
              ...
              bsync(fd,&c->task,RKNPU_MEM_SYNC_TO_DEVICE|RKNPU_MEM_SYNC_FROM_DEVICE); }
            if(submit1(c)){ rc_ret=-1; break; }   /* submit1: warmed c->Cc + correct domain (a hand-rolled submit gives bias-only) */
            int8_t*cc=c->Cc.cpu; for(int r=0;r<mc;r++)for(int n=0;n<Nc;n++) C[(size_t)(m0+r)*N+(n0+n)]=cc[(size_t)r*Nc+n];
        }
    }
    bdestroy(fd,&Gb);
...
int ork_npu_replay_softmax_f16(ork_npu *c, const void *in, void *out, double *us){
    int fd=c->fd;
    if(!ork_ppu_fuse_enabled(c)) return -3;
    struct buf IN=bcreate(fd,32768,0x403,-1), OUT=bcreate(fd,32768,0x403,-1),
               SCR=bcreate(fd,237568,0x403,-1), LUT=bcreate(fd,65536,0x403,-1),
               WT=bcreate(fd,(size_t)(SM_WT_WORDS+64)*4,0x403,-1);
    if(!IN.cpu||!OUT.cpu||!SCR.cpu||!LUT.cpu||!WT.cpu){
        bdestroy(fd,&IN);bdestroy(fd,&OUT);bdestroy(fd,&SCR);bdestroy(fd,&LUT);bdestroy(fd,&WT); return -2; }
    ...
    bsync(fd,&IN,RKNPU_MEM_SYNC_TO_DEVICE); bsync(fd,&OUT,RKNPU_MEM_SYNC_TO_DEVICE);
    bsync(fd,&SCR,RKNPU_MEM_SYNC_TO_DEVICE); bsync(fd,&LUT,RKNPU_MEM_SYNC_TO_DEVICE);
    bsync(fd,&WT,RKNPU_MEM_SYNC_TO_DEVICE);
    act(fd,RKNPU_ACT_RESET,0);
    ...
    for (int i = 0; i < S; i++) {
        ...
        for (int m0 = 0; m0 < M; m0 += mcap, p++) {
            ...
            if (kind == OP_SILU || kind == OP_EWMUL) {
                ...
                else if (ss && !ss->ops && ss->sdp_task >= 1 && i == ss->sdp_task - 1) set_i8_out8(rc, N, 0, ss->gate_mult, ss->gate_shift);   // legacy gate->silu
                if (validate_regcmd("run_chain_i8", c, rc, REGCMD_I8_N, w, extra, extra_n)) { ok = -1; goto cleanup; }
            }
...
int ork_mm_run_stream_i8(ork_npu *c, int S, const ork_mm_task_i8 *tasks) {
    ...
    if (c->last_dt != 3) { int kw=ork_f16warm()&&ORK_KW_DT(c->last_dt); if (!ORK_I8_LIVE(c->last_dt) && !kw) { act(fd, RKNPU_ACT_RESET, 0); c->warmed = 0; for (int i = 0; i < ORK_MAXCORE; i++) c->mwarm[i] = 0; } c->last_dt = 3; }
...
static void ssm_pool_free(ork_npu *c){
    if(c->ssm_pS) for(int h=0;h<c->ssm_nh;h++){ if(c->ssm_pS[h])ork_mm_free(c,c->ssm_pS[h]); if(c->ssm_pD&&c->ssm_pD[h])ork_mm_free(c,c->ssm_pD[h]); if(c->ssm_pC&&c->ssm_pC[h])ork_mm_free(c,c->ssm_pC[h]); if(c->ssm_pO&&c->ssm_pO[h])ork_mm_free(c,c->ssm_pO[h]); }
    ...
    c->ssm_nc=c->ssm_nr=c->ssm_nh=0;
}
/* Ensure the pool is allocated for (nc,nr,nh); reuse if the shape matches, else realloc. CS is fixed (64). */
static int ssm_pool_ensure(ork_npu *c,int nc,int nr,int nh){
    const int CS=64;
    if(c->ssm_pS && c->ssm_nc==nc && c->ssm_nr==nr && c->ssm_nh==nh) return 0;   /* warm reuse */
    ssm_pool_free(c);
    c->ssm_pS=calloc(nh,sizeof(ork_w*));c->ssm_pD=calloc(nh,sizeof(ork_w*));c->ssm_pC=calloc(nh,sizeof(ork_w*));c->ssm_pO=calloc(nh,sizeof(ork_w*));
    if(!c->ssm_pS||!c->ssm_pD||!c->ssm_pC||!c->ssm_pO){ ssm_pool_free(c); return -1; }
    for(int h=0;h<nh;h++){ c->ssm_pS[h]=ork_mm_f16_scratch(c,nc,CS); c->ssm_pD[h]=ork_mm_f16_scratch(c,CS,nr); c->ssm_pC[h]=ork_mm_f16_scratch(c,CS,nc); c->ssm_pO[h]=ork_mm_f16_scratch(c,nc,nr);
        if(!c->ssm_pS[h]||!c->ssm_pD[h]||!c->ssm_pC[h]||!c->ssm_pO[h]){ c->ssm_nh=nh; ssm_pool_free(c); return -1; } }
    c->ssm_aS=malloc((size_t)nh*CS*nc*2);c->ssm_bS=malloc((size_t)nh*nc*CS*2);c->ssm_G=malloc((size_t)nh*CS*CS*4);
    c->ssm_aD=malloc((size_t)nh*CS*CS*2);c->ssm_bD=malloc((size_t)nh*CS*nr*2);c->ssm_Yd=malloc((size_t)nh*CS*nr*4);
    c->ssm_aC=malloc((size_t)nh*nr*CS*2);c->ssm_bC=malloc((size_t)nh*CS*nc*2);c->ssm_cs=malloc((size_t)nh*nr*nc*4);
    c->ssm_aO=malloc((size_t)nh*CS*nc*2);c->ssm_bO=malloc((size_t)nh*nc*nr*2);c->ssm_tmp=malloc((size_t)nh*CS*nr*4);
    c->ssm_Acs=malloc((size_t)nh*CS*8);c->ssm_xbar=malloc((size_t)nh*CS*nr*8);
    c->ssm_state=malloc((size_t)nh*nr*nc*4);c->ssm_stp=malloc((size_t)nh*nr*nc*4);
    c->ssm_Acl=malloc(nh*8);c->ssm_Aclp=malloc(nh*8);c->ssm_tk=malloc((size_t)nh*sizeof(ork_mm_task_f16));
    ...
    if(c->ssm_i8&1){
        c->ssm_pSi8=calloc(nh,sizeof(ork_w*)); if(!c->ssm_pSi8){ c->ssm_nh=nh; ssm_pool_free(c); return -1; }
        int8_t *zero=calloc((size_t)nc*CS,1);
        for(int h=0;h<nh;h++){ c->ssm_pSi8[h]=ork_mm_pack_i8(c,nc,CS,zero); if(!c->ssm_pSi8[h]){ free(zero); c->ssm_nh=nh; ssm_pool_free(c); return -1; } }
        free(zero);
    ...
    c->ssm_nc=nc;c->ssm_nr=nr;c->ssm_nh=nh;
    return 0;
}
int ork_ssm_scan_f32(ork_npu *c,int nc,int nr,int nh,int ng,int nt,int ns,
    ...
    if(ssm_pool_ensure(c,nc,nr,nh)) return -1;   /* persistent pool: alloc once per shape, reuse across calls */
...
int ork_mm_run_i4_incr(ork_npu *c, ork_w *w, int M, const int8_t *A, int32_t *C){
    ...
    for(int blk=0; blk<NBLK; blk++){
        ...
        struct rknpu_submit sub;memset(&sub,0,sizeof sub);
        sub.flags=ork_ppflags();sub.task_number=(uint32_t)M;sub.task_obj_addr=sTK.obj;sub.fence_fd=-1;sub.core_mask=1u;
...
static void *i4_incr_mcworker(void *vp){
    ...
    for(int blk=b0; blk<b1 && !c->mc_error; blk++){
        ...
        struct rknpu_submit sub;memset(&sub,0,sizeof sub);
        sub.flags=ork_ppflags();sub.task_number=(uint32_t)M;sub.task_obj_addr=TK->obj;sub.fence_fd=-1;sub.core_mask=1u<<i;
        sub.subcore_task[0]=sub.subcore_task[1]=sub.subcore_task[2]=(struct rknpu_subcore_task){0,(uint32_t)M};
        sub.timeout=2000;
        int reps = c->mwarm[i]?1:2;
...
/* Per-core worker: N-chunk range [c0,c1) on core `core`. Chains this range's H-row batch-tasks
 * (M-group inner) in one core_mask=1<<core submit; packs its own A copy into maf[core]. */
struct i4bchw { ork_npu *c; int core, nc, M, H, Wb, NG, Wmax, c0, c1; ork_w *w; const int8_t *A; int32_t *C; int rc; };
static void *i4_bchain_worker(void *vp){
    struct i4bchw *a=vp; ork_npu *c=a->c; int i=a->core, fd=c->fd; pin_big_core(i);
    ork_w *w=a->w; int K=w->K, N=w->N, M=a->M, H=a->H, Wb=a->Wb, NG=a->NG, Wmax=a->Wmax, c0=a->c0, c1=a->c1;
    a->rc=0; if(c1<=c0) return NULL;
    int myNC=c1-c0, NT=myNC*NG;
    ...
    if(c->mtk[i].size<need_tk){ bdestroy(fd,&c->mtk[i]); c->mtk[i]=bcreate(fd,need_tk,0x40b,c->dom_active); }
    if(c->mccsz[i]<need_o){ bdestroy(fd,&c->mcc[i]); c->mcc[i]=bcreate(fd,need_o,0x403,c->dom_active); c->mccsz[i]=need_o; c->mwarm[i]=0; }
    if(!c->mrc[i].cpu||!c->maf[i].cpu||!c->mtk[i].cpu||!c->mcc[i].cpu){ a->rc=-2; c->mc_error=1; return NULL; }
    uint8_t *abase=c->maf[i].cpu; memset(abase,0,need_af);   /* A packed once per M-group (stride-2), reused over this core's chunks */
    ...
    bsync(fd,&c->maf[i],RKNPU_MEM_SYNC_TO_DEVICE);
    int tk=0;
    for(int nc2=c0;nc2<c1;nc2++){ int n0=nc2*Wb, Wc=(N-n0<Wb)?(N-n0):Wb;
        ...
        for(int g=0;g<NG;g++){ int Hg=(M-g*H<H)?(M-g*H):H; uint32_t rc[REGCMD_I4_N];
            ...
            synth_i4(rc, 2*Hg, K, Wc, aA, wdma, aC);
    ...
    bsync(fd,&c->mtk[i],RKNPU_MEM_SYNC_TO_DEVICE|RKNPU_MEM_SYNC_FROM_DEVICE);
    struct rknpu_submit sub; memset(&sub,0,sizeof sub);
    sub.flags=ork_ppflags(); sub.task_number=(uint32_t)NT; sub.task_obj_addr=c->mtk[i].obj; sub.core_mask=1u<<i; sub.fence_fd=-1;
    ...
    for(int nc2=c0;nc2<c1;nc2++){ int n0=nc2*Wb, Wc=(N-n0<Wb)?(N-n0):Wb, NBc=Wc/64;
        for(int g=0;g<NG;g++){ int Hg=(M-g*H<H)?(M-g*H):H; int16_t *og=o+(size_t)tk*(4*H*Wmax)*64;
            for(int j=0;j<Hg;j++) for(int b=0;b<NBc;b++){ size_t base=(size_t)(4*j+4*Hg*b)*64;
                int32_t *crow=a->C+(size_t)(g*H+j)*N+n0+b*64;
                for(int cc=0;cc<64;cc++) crow[cc]=og[base+cc]; }
            tk++; } }
    a->rc = c->mc_error ? -1 : 0; return NULL;
}
/* MULTI-CORE batch×chain: tile N by weight-bank width (131072/K), split the N-chunks across the cores;
 * each core chains its chunks' H-row batches in one core_mask=1<<i submit (weight resident per chunk),
 * writing disjoint output columns. Composes STRATEGY-A H-row batching + STRATEGY-C weight-load-once
 * chaining + 3-core N-parallelism. Bit-exact; single-bank chunks avoid the 0x1040 multi-block wall. */
static int run_i4_bchain(ork_npu *c, ork_w *w, int M, const int8_t *A, int32_t *C){
    ...
    if(c->last_dt!=DT_I4){ int keepwarm=ork_nothrash()&&ORK_INT_DT(c->last_dt);
        if(!keepwarm){ act(c->fd,RKNPU_ACT_RESET,0); for(int q=0;q<ORK_MAXCORE;q++)c->mwarm[q]=0; } c->last_dt=DT_I4; }
    if(mc_ensure(c,nc)) return -1;
    c->mc_error=0;
    struct i4bchw args[ORK_MAXCORE]; pthread_t th[ORK_MAXCORE];
    for(int i=0;i<nc;i++){ int a0=(int)((long)i*NC/nc), a1=(int)((long)(i+1)*NC/nc);
        args[i]=(struct i4bchw){c,i,nc,M,H,Wb,NG,Wmax,a0,a1,w,A,C,0}; }
    for(int i=1;i<nc;i++) pthread_create(&th[i],NULL,i4_bchain_worker,&args[i]);
    i4_bchain_worker(&args[0]);
    for(int i=1;i<nc;i++) pthread_join(th[i],NULL);
    for(int i=0;i<nc;i++) if(args[i].rc) return -1;
    return 0;
}

/* EXPERIMENTAL (ORK_I4_CBATCH): the "batching-chains" dual of BCHAIN. Per bank-width N-chunk, one FULL
 * synth_i4 H-row batch primes weight+config; each subsequent M-group is a LEAN COMPACT batch task that
 * REPLAYS the priming full's batch/config register VALUES (captured via regcmd_getv — no mc_phys guessing)
 * and only advances A/C, NOT re-issuing the weight-DMA config -> weight resident. ~19 regcfg vs full 116.
 * Single-core. -4 fallback.
 *
 * *** RK3588: NOT VIABLE — kept gated (default off) for other NPUs. *** A multi-row (mc_phys>1) compact
 * returns errno-110 (chain-walk timeout) for EVERY register config swept (ORK_I4_CB_REGS bitmask over the
 * batch regs + ORK_I4_CB_AMT chain-amount), while the same base format at mc=1 (the proven STRATEGY-C 1-row
 * compact) works. RK3588's compact incremental task is single-row-only — a batch needs the full task's
 * weight-DMA context (the vendor confirms: int4 batch = task_number=96 of 1-row compacts). So on RK3588
 * BCHAIN's full-task-per-batch is REQUIRED; CBATCH is retained only in case another NPU (e.g. RK3576)
 * accepts a multi-row compact. See wiki Exp-2026-07-08-INT4-BChain-Batch-Chain. */
static int run_i4_cbatch(ork_npu *c, ork_w *w, int M, const int8_t *A, int32_t *C){
    if(w->Sk!=1 || w->Sn!=1 || (w->N%64)) return -4;
    ...
    if(c->mccsz[i]<need_o){ bdestroy(fd,&c->mcc[i]); c->mcc[i]=bcreate(fd,need_o,0x403,c->dom_active); c->mccsz[i]=need_o; c->mwarm[i]=0; }
    if(!c->mrc[i].cpu||!c->maf[i].cpu||!c->mtk[i].cpu||!c->mcc[i].cpu) return -2;
    uint8_t *abase=c->maf[i].cpu; memset(abase,0,need_af);
    for(int g=0;g<NG;g++){ int Hg=(M-g*H<H)?(M-g*H):H;
        for(int j=0;j<Hg;j++) tile_i4_Aslice(abase+(size_t)(g*2*H+2*j)*(K/2), A+(size_t)(g*H+j)*K, 0, K); }
    bsync(fd,&c->maf[i],RKNPU_MEM_SYNC_TO_DEVICE);
    struct rknpu_task *t=c->mtk[i].cpu; memset(t,0,(size_t)NT*sizeof*t);
    uint32_t cv[16]={0}; static const struct{unsigned blk,reg;} CAP[16]={  /* batch/config regs replayed in compacts */
        {0x201,0x1040},{0x201,0x1104},{0x201,0x1100},{0x201,0x100c},{0x201,0x107c},{0x201,0x1020},{0x201,0x1084},
        {0x201,0x102c},{0x201,0x1010},{0x201,0x1088},{0x1001,0x4004},{0x1001,0x4034},{0x1001,0x4038},
        {0x1001,0x4058},{0x1001,0x405c},{0x801,0x3014} };
    int tk=0, nfull=(NG>=2)?2:1;                   /* 2 priming full batches/chunk before compacts (errno-110 fix) */
    for(int nc2=0;nc2<NC;nc2++){ int n0=nc2*Wb, Wc=(N-n0<Wb)?(N-n0):Wb;
        uint32_t wdma=(uint32_t)(w->Bb[0].dma + (uint64_t)(n0/64)*K*32);
        for(int g=0;g<NG;g++){ int Hg=(M-g*H<H)?(M-g*H):H;
            uint32_t aA=(uint32_t)c->maf[i].dma+(uint32_t)(g*2*H)*(K/2);
            uint32_t aC=(uint32_t)c->mcc[i].dma+(uint32_t)tk*(4*H*Wmax)*64*2;
            uint32_t *slot=(uint32_t*)((char*)c->mrc[i].cpu+(size_t)tk*REGCMD_I4_N*4);
            int last=(tk==NT-1); uint32_t nx=last?0:(uint32_t)(c->mrc[i].dma+(size_t)(tk+1)*REGCMD_I4_N*4);
            int next_full=(!last)&&(g+1<nfull || g==NG-1); uint32_t amt=next_full?0x0037:0x0007;
            if(g<nfull){                              /* priming FULL batch (g==0 captures reg values) */
                uint32_t rc[REGCMD_I4_N]; synth_i4(rc, 2*Hg, K, Wc, aA, wdma, aC);
                for(int q=0;q<16;q++) cv[q]=regcmd_getv(rc,REGCMD_I4_N,CAP[q].blk,CAP[q].reg);
                if(last){ rc[216]=0; rc[217]=0; rc[218]=0x00000014; rc[219]=0x01010000; }
                else { rc[216]=0x0010|((nx&0xffff)<<16); rc[217]=(0x0101<<16)|((nx>>16)&0xffff);
                    rc[218]=0x0014|(amt<<16); rc[219]=(0x0101<<16)|0; }
                memcpy(slot, rc, REGCMD_I4_N*4); t[tk].regcfg_amount=116;
            } else {                                  /* COMPACT batch: base-12 (STRATEGY-C order) + configurable extras */
                uint32_t *ic=slot;
                static int extra=-1, camt=-1;
                if(extra<0){ const char*e=getenv("ORK_I4_CB_REGS"); extra=e?(int)strtoul(e,0,0):0x7f; }
                if(camt<0){ const char*e=getenv("ORK_I4_CB_AMT"); camt=e?(int)strtoul(e,0,0):0x0007; }
                uint32_t cam=next_full?0x0037:(uint32_t)camt;
                #define SPC(ii,off,blk_,val) do{ic[2*(ii)]=(off)|(((uint32_t)(val)&0xffff)<<16); ic[2*(ii)+1]=((uint32_t)(blk_)<<16)|(((uint32_t)(val)>>16)&0xffff);}while(0)
                int p=0;   /* base 12 (proven STRATEGY-C set/order), 0x1084=captured batch mc, 0x4058=Wc-1 */
                SPC(p++,0x1040,0x0201,cv[0]); SPC(p++,0x1104,0x0201,0); SPC(p++,0x1100,0x0201,0); SPC(p++,0x100c,0x0201,cv[3]);
                SPC(p++,0x4004,0x1001,0x0e); SPC(p++,0x1070,0x0201,aA); SPC(p++,0x1084,0x0201,cv[6]); SPC(p++,0x1088,0x0201,(uint32_t)K);
                SPC(p++,0x1110,0x0201,wdma); SPC(p++,0x4020,0x1001,aC); SPC(p++,0x4058,0x1001,(uint32_t)(Wc-1)); SPC(p++,0x405c,0x1001,0);
                if(extra&0x01) SPC(p++,0x107c,0x0201,cv[4]);   /* activation-cube size */
                if(extra&0x02) SPC(p++,0x1020,0x0201,cv[5]);   /* M-count */
                if(extra&0x04) SPC(p++,0x102c,0x0201,cv[7]);
                if(extra&0x08) SPC(p++,0x1010,0x0201,cv[8]);   /* CNA hint */
                if(extra&0x10) SPC(p++,0x4034,0x1001,cv[11]);  /* PPU rows */
                if(extra&0x20) SPC(p++,0x4038,0x1001,cv[12]);  /* out width/4 */
                if(extra&0x40) SPC(p++,0x3014,0x0801,cv[15]);  /* DPU rows */
                int rega=p;
                if(last){ SPC(p,0x0010,0x0101,0); SPC(p+1,0x0014,0x0101,0); } else { SPC(p,0x0010,0x0101,nx); SPC(p+1,0x0014,0x0101,cam); }
                SPC(p+2,0x0000,0x0041,0); SPC(p+3,0x0008,0x0081,0x000d);
                #undef SPC
                t[tk].regcfg_amount=rega;
            }
            t[tk].enable_mask=0xd; t[tk].int_mask=0x300; t[tk].int_clear=0x1ffff; t[tk].regcmd_addr=c->mrc[i].dma+(uint64_t)tk*REGCMD_I4_N*4;
            tk++; } }
    bsync(fd,&c->mrc[i],RKNPU_MEM_SYNC_TO_DEVICE);
    bsync(fd,&c->mtk[i],RKNPU_MEM_SYNC_TO_DEVICE|RKNPU_MEM_SYNC_FROM_DEVICE);
    struct rknpu_submit sub; memset(&sub,0,sizeof sub);
...
static void NAME(T *dst, const T *src, int rows, int cols, long sr, long sc){ \
...
ORK_BMM_GATHER(bmm_gather_i8, int8_t)
ORK_BMM_GATHER(bmm_gather_f16, f16)
/* scatter contiguous src[r*cols+c] -> strided dst[r,c] = dst + r*sr + c*sc (C output; int32/float share size) */
static void bmm_scatter_i32(int32_t *dst, const int32_t *src, int rows, int cols, long sr, long sc){
...
static ork_bmm_strides bmm_natural(int M,int K,int N){
...
static int bmm_c_dense(const ork_bmm_strides *s,int N){ return s->cs_n==1 && s->cs_m==N; }
...
int ork_bmm_i8_strided(ork_npu *c, int nbatch, int M, int K, int N,
    ...
    int cdense=bmm_c_dense(s,N); int32_t *Cc = cdense?NULL:malloc((size_t)M*N*sizeof(int32_t));
    if(!Ac||!Bc||(!cdense&&!Cc)){ free(Ac);free(Bc);free(Cc); return -3; }
    bmm_gather_i8(Bc,B+s->abs*0+s->bbs*0,K,N,s->bs_k,s->bs_n); /* batch-0 B for the pack */
    ork_w *w = ork_mm_pack_i8(c, K, N, Bc);
    int rc=0;
    if(!w) rc=-3;
    for(int b=0;!rc&&b<nbatch;b++){
        if(b>0){ bmm_gather_i8(Bc,B+(long)b*s->bbs,K,N,s->bs_k,s->bs_n);
                 if(ork_mm_repack_i8(c,w,K,N,Bc)){ rc=-4; break; } }
        bmm_gather_i8(Ac,A+(long)b*s->abs,M,K,s->as_m,s->as_k);
        int32_t *Cout = cdense ? C+(long)b*s->cbs : Cc;
        if(ork_mm_run_i8(c,w,M,Ac,Cout)){ rc=-5; break; }
        if(!cdense) bmm_scatter_i32(C+(long)b*s->cbs,Cc,M,N,s->cs_m,s->cs_n);
    }
...
int ork_bmm_i4_strided(ork_npu *c, int nbatch, int M, int K, int N,
    ...
    for(int b=0;b<nbatch;b++){
        bmm_gather_i8(Bc,B+(long)b*s->bbs,K,N,s->bs_k,s->bs_n);
        bmm_gather_i8(Ac,A+(long)b*s->abs,M,K,s->as_m,s->as_k);
...
int ork_bmm_fp16_strided(ork_npu *c, int nbatch, int M, int K, int N,
    ...
    int cdense=bmm_c_dense(s,N); float *Cc = cdense?NULL:malloc((size_t)M*N*sizeof(float));
    if(!Ac||!Bc||(!cdense&&!Cc)){ free(Ac);free(Bc);free(Cc); return -3; }
    int rc=0;
    for(int b=0;b<nbatch;b++){
        bmm_gather_f16(Bc,B+(long)b*s->bbs,K,N,s->bs_k,s->bs_n);
        bmm_gather_f16(Ac,A+(long)b*s->abs,M,K,s->as_m,s->as_k);
        ork_w *w=ork_mm_pack(c,K,N,Bc); if(!w){ rc=-3; break; }
        float *Cout = cdense ? C+(long)b*s->cbs : Cc;
        int r=ork_mm_run(c,w,M,Ac,Cout);
        ork_mm_free(c,w);
        if(r){ rc=-5; break; }
        if(!cdense) bmm_scatter_i32((int32_t*)(C+(long)b*s->cbs),(const int32_t*)Cc,M,N,s->cs_m,s->cs_n);
    }
    free(Ac);free(Bc);free(Cc);
    return rc;
}
...
int ork_bmm_i8(ork_npu *c, int nbatch, int M, int K, int N,
    ...
    ork_bmm_strides s=bmm_natural(M,K,N); return ork_bmm_i8_strided(c,nbatch,M,K,N,A,B,C,&s);
}
int ork_bmm_i4(ork_npu *c, int nbatch, int M, int K, int N,
               const int8_t *A, const int8_t *B, int32_t *C){
    ork_bmm_strides s=bmm_natural(M,K,N); return ork_bmm_i4_strided(c,nbatch,M,K,N,A,B,C,&s);
}
int ork_bmm_fp16(ork_npu *c, int nbatch, int M, int K, int N,
                 const f16 *A, const f16 *B, float *C){
    ork_bmm_strides s=bmm_natural(M,K,N); return ork_bmm_fp16_strided(c,nbatch,M,K,N,A,B,C,&s);
}
