...
#define ORK_OP_TABLE(X) \
  /* --- pack / import / free ------------------------------------------------------------------ */ \
  X(pack_i8,      ork_mm_pack_i8,       orkd_pack_i8)      \
  X(pack_f16,     ork_mm_pack,          orkd_pack_f16)     \
  X(pack_i4,      ork_mm_pack_i4,       orkd_pack_i4)      \
  X(free,         ork_mm_free,          orkd_free_weight)  \
  /* --- matmul run --------------------------------------------------------------------------- */ \
  X(run_i8,       ork_mm_run_i8,        orkd_run_i8)       \
  X(run_f16,      ork_mm_run,           orkd_run_f16)      \
  X(run_i4,       ork_mm_run_i4,        orkd_run_i4)       \
  X(run_chain_i8, ork_mm_run_chain_i8,  orkd_run_chain_i8) \
  ...
  X(layer_i8,     ork_mm_layer_i8,      orkd_layer_i8)     \
  /* --- resident KV -------------------------------------------------------------------------- */ \
  X(kv_append,    ork_kv_append,        orkd_kv_append)    \
  /* --- SDP activation ops (int8) ------------------------------------------------------------ */ \
  X(silu_i8,      ork_npu_silu_i8,      orkd_silu_i8)      \
  X(gelu_i8,      ork_npu_gelu_i8,      orkd_gelu_i8)      \
  X(ewmul_i8,     ork_npu_ewmul_i8,     orkd_ewmul_i8)     \
  X(add_i8,       ork_npu_add_i8,       orkd_add_i8)       \
  X(rsqrt_i8,     ork_npu_rsqrt_i8,     orkd_rsqrt_i8)     \
  X(exp_i8,       ork_npu_exp_i8,       orkd_exp_i8)       \
  /* --- SDP activation ops (int16) ----------------------------------------------------------- */ \
  X(silu_i16,     ork_npu_silu_i16,     orkd_silu_i16)     \
  X(gelu_i16,     ork_npu_gelu_i16,     orkd_gelu_i16)     \
  X(ewmul_i16,    ork_npu_ewmul_i16,    orkd_ewmul_i16)    \
  X(add_i16,      ork_npu_add_i16,      orkd_add_i16)      \
  X(rsqrt_i16,    ork_npu_rsqrt_i16,    orkd_rsqrt_i16)    \
  X(exp_i16,      ork_npu_exp_i16,      orkd_exp_i16)
