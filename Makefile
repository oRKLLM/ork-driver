...
PREFIX ?= /usr/local
CORE := src/npu.c src/npu/core/buf.c src/npu/core/device.c src/npu/core/domain.c src/npu/core/mode.c src/npu/core/prof.c src/npu/core/sched.c src/npu/core/submit.c src/npu/f16/pack.c src/npu/f16/perchan.c src/npu/f16/probe.c src/npu/f16/regcmd.c src/npu/f16/replay.c src/npu/f16/run.c src/npu/f16/stream.c src/npu/i16/act.c src/npu/i16/chain.c src/npu/i16/probe.c src/npu/i16/regcmd.c src/npu/i4/chain.c src/npu/i4/pack.c src/npu/i4/quant.c src/npu/i4/run.c src/npu/i4/stream.c src/npu/i8/chain.c src/npu/core/colsplit.c src/npu/core/dyn.c src/npu/i8/dyn_seq.c src/npu/core/dyn_ctl.c src/npu/i8/queue.c src/npu/i8/fold.c src/npu/i8/pack.c src/npu/i8/probe.c src/npu/i8/probe_sdp.c src/npu/i8/probe_replay.c src/npu/i8/probe_prof.c src/npu/i8/probe_chain.c src/npu/i8/regcmd.c src/npu/i8/run.c src/npu/norm.c src/npu/sdp.c src/npu/ssm.c src/soc.c src/soc/rk3588.c src/soc/rk3576.c src/neon_activations.c src/ork_ops.c
...
libork_npu.so: $(COBJ) # shared — dynamic link / FFI from Python, Node, Rust, ...
	$(CC) $(CFLAGS) -shared -o $@ $^
# HEADER DEPENDENCIES (-MMD -MP). Without these a header edit rebuilds NOTHING that includes it, so the
# link silently mixes objects compiled against DIFFERENT struct layouts. Adding a field to struct ork_npu
# (internal.h) did exactly that on 2026-08-25: stale objects kept the old offsets, and the mismatch surfaced
# as a worker-pool deadlock (quant blocked in pthread_cond_wait inside ork_dyn_begin_colsplit, NPU idle, no
# faults) that looked for hours like a hardware/orkd problem. `make clean` made it pass. The compiler can
# track this for free -- it should, rather than every header edit being a silent correctness hazard.
...
# i4_group_probe — cost of a PER-GROUP int4 scale: narrow matmuls + the cross-group reduction, chained vs
# separate submits. Diagnostic (board only, not in all/test): answers whether the reduction is DRAM-bound
# (a faster dispatcher cannot help) or dispatch-bound (doorbell/heterogeneous overlap is the lever).
...
# Generated by -MMD: one .d per object listing the headers it includes, so touching a header rebuilds
# exactly its dependents. Missing on a fresh tree, hence the leading dash.
-include $(COBJ:.o=.d)
