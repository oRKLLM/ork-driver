 ...
 * @param w Weight from ork_i8_mm_pack().
 ...
 */
int          ork_i8_mm_run(ork_npu *ctx, ork_w *w, int M, const int8_t  *A, int32_t *C);
...
int          ork_i8_mm_run_silu(ork_npu *ctx, ork_w *w, int M, const int8_t *A, int8_t *C,
                                int r_mult, int r_shift, unsigned out_bias, unsigned idx_off,
                                unsigned cfg4068, const short *lut, int nlut);
/* Same as ork_i8_mm_run_silu but INT32 output (silu NOT quantized to int8): out_i32 = R*V16[idx]+out_bias,
...
int          ork_i8_mm_run_silu32(ork_npu *ctx, ork_w *w, int M, const int8_t *A, int *C,
                                  int r_mult, int r_shift, unsigned out_bias, unsigned idx_off,
                                  unsigned cfg4068, const short *lut, int nlut);
 ...
 * gated OFF, built for a future int8-win pipeline). w = fp16 weight (ork_f16_mm_pack), A = fp16 [M,K], C = fp32
 * [M,N] silu(gate). K%32, N%16, N<=nmax. 0/ok,-1,-2,-3. WIP: fp16 LUT calibration approximate. */
int          ork_f16_mm_run_silu(ork_npu *ctx, ork_w *w, int M, const ork_f16 *A, float *C,
...
int          ork_f16_mm_build_lut(ork_npu *ctx, double (*fn)(double, void *), void *fnctx,
...
int          ork_f16_mm_run_act(ork_npu *ctx, int K, int N, const ork_f16 *B, int M, const ork_f16 *A, float *C,
 ...
 * bakes the calibrated LUT + out-scale into the weight (ork_f16_mm_pack_fused_act), run replays it with no
 * re-pack (ork_f16_mm_run_fused_act, C[M,N]=fn(A·B), one submit). Same single-signed band rule as _act; the
 * run path carries no fn pointer (crosses a seq/socket). NULL/-2 on bad shape/mixed-sign/no-baked-LUT. */
ork_w       *ork_f16_mm_pack_fused_act(ork_npu *ctx, int K, int N, const ork_f16 *B,
                                       double (*fn)(double, void *), void *fnctx, double in_lo, double in_hi);
int          ork_f16_mm_run_fused_act(ork_npu *ctx, ork_w *w, int M, const ork_f16 *A, float *C);
/* Calibrate the fp16 fused SiLU for a gate spanning [-Gmax,Gmax] (caps Gmax → the fp16 spread band, then
 * ork_f16_mm_build_lut). silu(gate)=C_out*out_scale; pack the gate weight as -S*W. See Exp-2026-07-05. */
int          ork_f16_mm_build_silu_lut(ork_npu *ctx, double Gmax, short *lut,
                                       double *S_out, double *R_out, double *out_scale_out);
 ...
 * rsqrt; the int8/int16 STANDALONE rsqrt of a tensor is ork_i8_npu_rsqrt/i16 (act_lut_i8/i16, same rsqrt_f).
 * Same op, different precision + fusion regime — both kept. */
int          ork_f16_mm_build_rsqrt_lut(ork_npu *ctx, int n_feat, double eps, double ss_min, double ss_max,
 ...
 * multiply by G (= silu(gate) from ork_i8_mm_run_silu) applied IN the up matmul's SDP output stage.
 * gain = mult/2^shift = s_up*s_silu/s_out. G is dense int8 [M*N]. Resident full-K int8 weight
 * (K%512==0, K<=4096). Together with ork_i8_mm_run_silu this is the full fused SwiGLU FFN inner. */
int          ork_i8_mm_run_ewmul(ork_npu *ctx, ork_w *w, int M, const int8_t *A, const int8_t *G,
...
int          ork_i8_mm_run_out8(ork_npu *ctx, ork_w *w, int M, const int8_t *A, int8_t *C,
                                int mult, int shift);
...
int          ork_i8_mm_run_out16(ork_npu *ctx, ork_w *w, int M, const int8_t *A, short *C, int mult, int shift);
...
int          ork_i4_mm_run(ork_npu *ctx, ork_w *w, int M, const int8_t  *A, int32_t *C);
 ...
 * synchronous ork_f16_mm_run* which auto-falls back to the socket). Keeping several ops in flight (up to the ring
 ...
 * with ork_i4_mm_pack_grouped. (Cost: K/G submits/core — larger G = fewer submits, coarser scale.) */
