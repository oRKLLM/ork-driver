...
int          ork_i8_npu_probe_out8(ork_npu *ctx, int M, int K, int N, const int8_t *A, const int8_t *B,
 ...
 * register config; run matmuls via ork_i8_npu_probe_silu_cfg(..,r_mult,r_shift,0,0xffffc000,cfg4068,lut,1030,..).
...
int          ork_i8_npu_probe_silu_cfg(ork_npu *ctx, int M, int K, int N, const int8_t *A, const int8_t *B,
...
int          ork_i8_npu_ewmul(ork_npu *ctx, const int8_t *up, const int8_t *silu, int M, int N,
...
int          ork_f16_npu_ewmul(ork_npu *ctx, const ork_f16 *up, const ork_f16 *silu, int M, int N,
...
int          ork_i16_npu_ewmul(ork_npu *ctx, const int16_t *up, const int16_t *silu, int M, int N,
...
int          ork_f16_npu_rmsnorm(ork_npu *ctx, int M, int n, const ork_f16 *x, const ork_f16 *w, float eps, ork_f16 *out);
...
int          ork_f16_npu_rope_neox(ork_npu *ctx, const ork_f16 *x, int hd, int nrow, const int *pos, double freq_base, ork_f16 *out);
int          ork_f16_npu_l2norm (ork_npu *ctx, int M, int n, const ork_f16 *x,                     float eps, ork_f16 *out);
...
int          ork_i16_npu_chain_gatesilu(ork_npu *ctx, int M, int K, int N, const signed char *A, const signed char *B,
...
int          ork_i8_npu_probe_silu_std(ork_npu *ctx, const signed char *in, int M, int N,
...
int          ork_f16_npu_probe_silu_std(ork_npu *ctx, const ork_f16 *in, int M, int N,
...
int          ork_i16_npu_probe_silu_std(ork_npu *ctx, const short *in, int M, int N,
                                        int r_mult, int r_shift, unsigned out_bias, unsigned idx_off,
                                        unsigned cfg4064, unsigned cfg4068, const short *lut, int nlut,
                                        short *out, double *us);
...
int          ork_i8_npu_silu(ork_npu *ctx, const signed char *in, int M, int N,
...
int          ork_i8_npu_replay(ork_npu *ctx, const unsigned *regcmd, int rn, int M, int K, int N,
...
int          ork_i8_npu_fold_op(ork_npu *ctx, int K, int N, const signed char *Wraw, int M,
                                const signed char *Araw, int *Cout, int iters, double *us);
/* #39 mfold RESIDENT-weight variant: fold matmul from a PRE-PACKED w->Bfold (ork_i8_mm_load_fold / orkpack v5),
...
int          ork_i16_npu_silu(ork_npu *ctx, const short *in, int M, int N,
...
int          ork_i16_npu_chain_mm_silu(ork_npu *ctx, const short *in, int M, int N,
...
int          ork_i8_npu_row_max(ork_npu *ctx, const signed char *a, int M, int N, signed char *out, double *us);
...
int          ork_i8_npu_mul_perchan(ork_npu *ctx, const signed char *a, const signed char *b, int M, int N, int mult, int shift, signed char *out, double *us);
/* fp16 per-channel scale: out[m][n]=a[m][n]*b[n], b[N] broadcast across rows (ERDMA_DATA_MODE=0). N%8. Quant-free. */
int          ork_f16_npu_mul_perchan(ork_npu *ctx, const ork_f16 *a, const ork_f16 *b, int M, int N, ork_f16 *out, double *us);
/* int16 per-channel scale: out[m][n]=clamp_i16(a[m][n]*b[n]*mult>>shift), b[N] broadcast. N%8. Chain intermediate. */
int          ork_i16_npu_mul_perchan(ork_npu *ctx, const short *a, const short *b, int M, int N, int mult, int shift, short *out, double *us);
int          ork_i8_npu_add(ork_npu *ctx, const signed char *a, const signed char *b, int M, int N,
...
int          ork_f16_npu_add(ork_npu *ctx, const ork_f16 *a, const ork_f16 *b, int M, int N, ork_f16 *out, double *us);
...
int          ork_i8_npu_gelu(ork_npu *ctx, const signed char *in, int M, int N,
                             double in_scale, double out_scale, signed char *out, double *us);
/* On-NPU GELU (int16 / w16a16i): same op, GELU curve. RKNN-class accuracy. N%8==0. 0/ok,-1,-2,-3. */
int          ork_i16_npu_gelu(ork_npu *ctx, const short *in, int M, int N,
                              double in_scale, double out_scale, short *out, double *us);
...
int          ork_i8_npu_rsqrt(ork_npu *ctx, const signed char *in, int M, int N, double in_scale, double out_scale, signed char *out, double *us);
int          ork_i16_npu_rsqrt(ork_npu *ctx, const short *in, int M, int N, double in_scale, double out_scale, short *out, double *us);
/* On-NPU exp (int8/int16) — softmax building block, activation-LUT (exp curve): out=clamp(round(exp(in*is)/os)). */
int          ork_i8_npu_exp(ork_npu *ctx, const signed char *in, int M, int N, double in_scale, double out_scale, signed char *out, double *us);
...
int          ork_i8_npu_exp_biased(ork_npu *ctx, const signed char *in, int M, int N, double in_scale, double out_scale, double max, signed char *out, double *us);
int          ork_i16_npu_exp(ork_npu *ctx, const short *in, int M, int N, double in_scale, double out_scale, short *out, double *us);
/* Composed fused softmax over each row of [M][n] (fp16): out = exp(x-max)/Σexp(x-max). Gated ORK_SOFTMAX_NPU:
 * exp (ork_i16_npu_exp) + the Σ reduction (reduce-matmul) run on the NPU, max + normalize on CPU; falls back
 * to a full CPU softmax when the gate/PPU-fuse is off or n%32!=0. 0/ok, -2 bad args. */
int          ork_f16_npu_softmax(ork_npu *ctx, int M, int n, const ork_f16 *x, ork_f16 *out);
...
int          ork_i16_npu_add(ork_npu *ctx, const short *a, const short *b, int M, int N,
...
int          ork_i8_npu_probe_add(ork_npu *ctx, const signed char *a, const signed char *b, int M, int N,
...
int          ork_i4_npu_probe_mm(ork_npu *ctx, int M, int K, int N,
...
int          ork_f16_npu_probe_mm_f16out(ork_npu *ctx, int M, int K, int N,
