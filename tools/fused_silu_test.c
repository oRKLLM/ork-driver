 ...
 * silicon-validated probe: ork_i8_mm_run_silu(pack(B),A) == ork_i8_npu_probe_silu(A,B), same scales.
 * The probe is the bit-exact reference; no absolute-scale calibration needed.
 *   ork_i8_mm_run_silu  = gate matmul with SiLU fused in the SDP output stage  -> VALIDATED bit-exact.
 *   ork_i8_mm_run_ewmul = up matmul with the EW-mul (xsilu(gate)) fused in the output stage -> the
...
int main(void){
    ...
    { int32_t *cm=malloc((size_t)M*N*4); int r0=ork_i8_mm_run(c,w,M,A,cm); long mism=0;
    ...
    { int8_t *ref=malloc(M*N), *run=malloc(M*N);
      int r1=ork_i8_npu_probe_silu(c,M,K,N,A,B,ref,NULL);
      int r2=ork_i8_mm_run_silu(c,w,M,A,run,0x51aa,0x14,0xffffff9fu,0xffffc000u,0x56391100u,NULL,0);
    ...
    if(getenv("ORK_TEST_EWMUL")){
        ...
        int r3=ork_i8_npu_probe_ewmul(c,M,K,N,A,B,G,0x4000,14,er,NULL);
        int r4=ork_i8_mm_run_ewmul(c,w,M,A,G,rr,0x4000,14);
