 ...
 * applies per-channel scale, so it can feed directly off ork_f16_npu_probe_mm_f16out with no repack.
...
int main(void){
    ...
    double us=0;
    int rc=ork_f16_npu_mul_perchan_contig(c,A,scale,M,N,out,&us);
    int bad=0,nz=0;
