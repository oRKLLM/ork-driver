 ...
 *       1 = PLAIN fp16 ork_f16_mm_run only (the CPUSILU gate's matmul call) — no mode switch
 *       2 = PLAIN fp16 ork_f16_mm_run with core-budget SWITCH each iter (1 -> cores), mimicking CPUSILU gate/up */
int main(int argc,char**argv){
    ...
    ork_w*wi=ork_i8_mm_pack(c,K,Ni,Bi); if(!wi){printf("int8 pack fail\n");return 2;}
    ...
    ork_w*wf=ork_f16_mm_pack(c,K,Nf,Bf); if(!wf){printf("fp16 pack fail\n");return 2;}
    ...
    printf("  mode=%d (%s) ncore=%d\n", mode,
        mode==0?"int8<->f16_silu":mode==1?"plain-fp16 ork_f16_mm_run":"plain-fp16 + core-budget switch", ncore);
    for(int it=0; it<iters; it++){
        int r1=0,r2=0;
        if(mode==0 || mode==4){   /* mode 4 = same calls, but weights in 2 domains -> dom_activate each run */
            printf("  iter %2d int8...",it); fflush(stdout); r1=ork_i8_mm_run(c,wi,M,Ai,Ci);
            printf(" f16silu...");          fflush(stdout); r2=ork_f16_mm_run_silu(c,wf,M,Af,Cf,0,0xffffc000u,0x56391100u,NULL,0);
        } else if(mode==1){
            printf("  iter %2d f16run...",it); fflush(stdout); r2=ork_f16_mm_run(c,wf,M,Af,Cf);
        } else if(mode==2){
            printf("  iter %2d cb1 f16run...",it); fflush(stdout);
            ork_npu_set_core_budget(c,1);     r1=ork_f16_mm_run(c,wf,M,Af,Cf);
            printf(" cbN f16run...");          fflush(stdout);
            ork_npu_set_core_budget(c,ncore); r2=ork_f16_mm_run(c,wf,M,Af,Cf);
        } else {   /* mode 3: int8 CHAIN <-> fp16 — the actual model mixed path (chain "always resets") */
            ork_mm_task_i8 t={wi,M,Ai,Ci};
            printf("  iter %2d chain_i8...",it); fflush(stdout); r1=ork_i8_mm_run_chain(c,1,&t);
            printf(" f16silu...");              fflush(stdout); r2=ork_f16_mm_run_silu(c,wf,M,Af,Cf,0,0xffffc000u,0x56391100u,NULL,0);
        }
        printf(" ok (r1=%d r2=%d)\n",r1,r2); fflush(stdout);
