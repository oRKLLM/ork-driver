 ...
 *   rsqrt: fn=rsqrt via ork_mm_run_f16_act (positive-input; RMSNorm 1/sqrt(ss)).
 * (silu is the both-sign case via ork_mm_run_f16_silu + build_f16_silu_lut — separate, next.)
...
int main(int argc,char**argv){
    ...
    int rc=ork_mm_run_f16_act(c,d,N,B,M,A,C,myrsqrt,NULL,(double)lo-0.5,(double)hi+0.5);
    printf("  ork_mm_run_f16_act(fn=rsqrt) rc=%d C[0]=%.5f (want 1/sqrt(%.2f)=%.5f)\n",rc,C[0],sc[0],1/sqrt(sc[0]));
    ...
    { int Kd=128, Ns=64, Ms=32; double Gmax=8.0;
      ...
      int brc=ork_mm_build_f16_silu_lut(c,Gmax,lut,&S,&R,&os);
      ...
      ork_w *wg=ork_mm_pack(c,Kd,Ns,negW); if(!wg){ printf("FAIL (silu pack)\n"); ork_npu_free(c); return 1; }
      float *Cs=malloc((size_t)Ms*Ns*4);
      int src=ork_mm_run_f16_silu(c,wg,Ms,Ag,Cs,0,0xffffc000u,0x56391100u,lut,1030);
