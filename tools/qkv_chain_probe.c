 ...
 * (3 matmul tasks, all reading A) and compare to 3 separate ork_mm_run_i8 submits. GQA dims (Q wide, K/V narrow).
...
int main(int argc,char**argv){
    ...
    ork_w *wq=ork_mm_pack_i8(c,K,Nq,Wq), *wk=ork_mm_pack_i8(c,K,Nkv,Wk), *wv=ork_mm_pack_i8(c,K,Nkv,Wv);
    ...
    int r=ork_mm_run_chain_i8(c,3,t);
    ...
    double t0=now_us(); for(int i=0;i<ITER;i++) ork_mm_run_chain_i8(c,3,t); double ch=(now_us()-t0)/ITER;
    printf("  CHAIN (Q,K,V ONE submit): %8.1f us  bit-exact=%s\n", ch, okc?"YES":"NO");

    /* ---- SEPARATE: 3 submits ---- */
    ork_mm_run_i8(c,wq,M,A,Cq); ork_mm_run_i8(c,wk,M,A,Ck); ork_mm_run_i8(c,wv,M,A,Cv);
    int oks=1; for(int i=0;i<M*Nq;i++)if(Cq[i]!=K){oks=0;break;}
    t0=now_us(); for(int i=0;i<ITER;i++){ ork_mm_run_i8(c,wq,M,A,Cq); ork_mm_run_i8(c,wk,M,A,Ck); ork_mm_run_i8(c,wv,M,A,Cv); } double sep=(now_us()-t0)/ITER;
    ...
    int rs=ork_mm_run_stream_i8(c,3,t);
    if(!rs){
        int okr=1; for(int i=0;i<M*Nq&&okr;i++)if(Cq[i]!=K)okr=0; for(int i=0;i<M*Nkv&&okr;i++)if(Ck[i]!=K||Cv[i]!=K)okr=0;
        t0=now_us(); for(int i=0;i<ITER;i++) ork_mm_run_stream_i8(c,3,t); double st=(now_us()-t0)/ITER;
    ...
    ork_npu_set_pack_domain(c,0); ork_w*wcat=ork_mm_pack_i8(c,K,Ncat,Wcat);
    if(wcat){ int32_t*Cc=malloc((size_t)M*Ncat*4);
        ork_mm_run_i8(c,wcat,M,A,Cc);
        int okg=1; for(int i=0;i<M*Ncat&&okg;i++)if(Cc[i]!=K)okg=0;
        t0=now_us(); for(int i=0;i<ITER;i++) ork_mm_run_i8(c,wcat,M,A,Cc); double gr=(now_us()-t0)/ITER;
