 ...
 * ork_i8_mm_run_chain_gsilu (built on the PROVEN run_chain_i8 + set_i8_silu fused output stage). Validates the
...
int main(int argc, char**argv){
    ...
    ork_w *wg=ork_i8_mm_pack(c,K,N,Wg), *wu=ork_i8_mm_pack(c,K,N,Wu);
    ...
    if(getenv("ORK_GSILU_FFN6K")){   /* REAL-WIDTH FFN inner: Nff=6144 -> down K-splits (Sk=6) IN the chain */
        ...
        ork_w *wg6=ork_i8_mm_pack(c,Kf,Nff,Wg6);   /* gate/up [512,6144] */
        ork_w *wd6=ork_i8_mm_pack(c,Nff,Kd,Wd6);   /* down [6144,512], Sk=6 */
        ...
        int r=ork_i8_mm_run_chain_ffn(c,5,t,ops,is,os);
    ...
    if(getenv("ORK_GSILU_FFN5")){   /* FULL FFN inner: [gate -> silu -> up -> glu -> down] 5-task one submit */
        ...
        ork_w *w5=ork_i8_mm_pack(c,Kf,Nf,W5);   /* one all-ones [512,512] weight reused for gate/up/down */
        ...
        double is = 3.0/32.0, os = siluf(3.0)/60.0;
        int r=ork_i8_mm_run_chain_ffn(c,5,t,ops,is,os);
        if(r==0){ double t0=now_us(); for(int i=0;i<ITER;i++) ork_i8_mm_run_chain_ffn(c,5,t,ops,is,os);
    ...
    if(getenv("ORK_GSILU_FFN4")){   /* [gate -> silu -> up -> glu] 4-task one submit (matmul/SDP mix, aliased) */
        ...
        double is = 3.0/32.0, os = siluf(3.0)/60.0;
        int r=ork_i8_mm_run_chain_ffn(c,4,t,ops,is,os);
    ...
    if(getenv("ORK_GSILU_SDP")){   /* OPTION B: [gate matmul(int8-out) -> silu-SDP] one submit (vendor matmul->SDP) */
        ...
        int r=ork_i8_mm_run_chain_sdpsilu(c,2,t,1, 0x4000,18, is,os);
    ...
    if(getenv("ORK_GSILU_LAST")){     /* fused gate*silu as the LAST task: [up(plain,task0) -> gate*silu(task1)] */
        ...
        r=ork_i8_mm_run_chain_gsilu(c,2,t,1, 0x4000,0x10,0,0xffffc000u,0x56391300u, lut,1030);
    ...
    if(getenv("ORK_GSILU_PLAIN")){    /* bisection: ss=NULL plain chain (both int32) -- verify refactor didn't break run_chain_i8 */
        r=ork_i8_mm_run_chain(c,2,tasks);
    ...
    r=ork_i8_mm_run_chain_gsilu(c,2,tasks,0, 0x4000,0x10,0,0xffffc000u,0x56391300u, lut,1030);
