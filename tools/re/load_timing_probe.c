/* load_timing_probe.c — measure ork_mm_load_i8_import load cost per weight WITHOUT the full-model memory
 * pressure that wedges the board. Packs a representative gate/up-shape weight (K=3584 N=18944, the Bf-heavy
 * case: K<=4096 so it rebuilds full-K Bf), dumps the pre-tiled blob, then times ork_mm_load_i8_import of it
...
int main(int argc,char**argv){
    ...
    ork_w*w0=ork_mm_pack_i8(c,K,N,B); if(!w0){printf("pack failed\n");return 1;}
