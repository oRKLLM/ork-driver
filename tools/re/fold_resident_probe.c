 ...
 * weight (ork_i8_w_dump_fold_cpu -> ork_i8_mm_load_fold -> ork_npu_fold_run_w, NO per-call fold_woff repack)
 * vs the normal ork_i8_mm_run path, at the 7B q/k/v/o shapes (K=3584). A/B time + BIT-EXACT check.
 *
...
int main(int argc,char**argv){
    ...
    printf("fold_resident_probe: K=%d iters=%d  (resident mfold vs normal ork_i8_mm_run, q/k/v/o shapes)\n",K,iters);
    for(size_t ni=0; ni<sizeof Ns/sizeof*Ns; ni++){
        ...
        ork_w *wn=ork_i8_mm_pack(c,K,N,W);
        /* fold weight: dump -> load resident */
        size_t fb=ork_i8_w_dump_fold_cpu(c,K,N,W,NULL,0);
        void *blob=malloc(fb); ork_i8_w_dump_fold_cpu(c,K,N,W,blob,fb);
        ork_w *wf=ork_i8_mm_load_fold(c,K,N,blob,fb);
        ...
        for(size_t mi=0; mi<sizeof Ms/sizeof*Ms; mi++){
            ...
            if(ork_i8_mm_run(c,wn,M,A,Cn)){ printf("  M=%-3d normal run FAIL\n",M); free(Cn);free(Cf); continue; }
            ...
            /* time normal */
            double t0=now_us(); for(int i=0;i<iters;i++) ork_i8_mm_run(c,wn,M,A,Cn); double us_n=(now_us()-t0)/iters;
    ...
    /* ---- AUTO-ROUTE: a normally-packed q/o weight with a fold blob attached; ork_i8_mm_run must self-select
    ...
    { int N=3584; int8_t *W=malloc((size_t)K*N), *A=malloc((size_t)128*K);
      ...
      ork_w *wref=ork_i8_mm_pack(c,K,N,W), *wrt=ork_i8_mm_pack(c,K,N,W);
      size_t fb=ork_i8_w_dump_fold_cpu(c,K,N,W,NULL,0); void *blob=malloc(fb); ork_i8_w_dump_fold_cpu(c,K,N,W,blob,fb);
      int at=ork_i8_w_attach_fold(c,wrt,blob,fb);
      printf("\n== AUTO-ROUTE (N=3584): ork_i8_mm_run self-select (attach rc=%d) ==\n", at);
      int RM[]={32,128};
      for(size_t j=0;j<sizeof RM/sizeof*RM;j++){ int M=RM[j];
        int32_t *Cr=calloc((size_t)M*N,4), *Ce=calloc((size_t)M*N,4);
        ork_i8_mm_run(c,wref,M,A,Ce); ork_i8_mm_run(c,wrt,M,A,Cr);
        long mism=0; for(size_t e=0;e<(size_t)M*N;e++) if(Cr[e]!=Ce[e]){ mism++; }
        double t0=now_us(); for(int i=0;i<iters;i++) ork_i8_mm_run(c,wrt,M,A,Cr);  double us=(now_us()-t0)/iters;
        double t1=now_us(); for(int i=0;i<iters;i++) ork_i8_mm_run(c,wref,M,A,Ce); double un=(now_us()-t1)/iters;
     ...
     * normal ork_i8_mm_run. bit-exact + timing. This is where amortizing the nc16 marshal should pay off. ---- */
    { int K3=3584; int QN[3]={3584,512,512}; const char*nm[3]={"q","k","v"};
      ...
      for(int j=0;j<3;j++){ Wq[j]=malloc((size_t)K3*QN[j]); for(size_t i=0;i<(size_t)K3*QN[j];i++) Wq[j][i]=r8();
        wn[j]=ork_i8_mm_pack(c,K3,QN[j],Wq[j]);
        size_t fb=ork_i8_w_dump_fold_cpu(c,K3,QN[j],Wq[j],NULL,0); void*bl=malloc(fb); ork_i8_w_dump_fold_cpu(c,K3,QN[j],Wq[j],bl,fb);
        wfd[j]=ork_i8_mm_load_fold(c,K3,QN[j],bl,fb); free(bl);
        if(!wn[j]||!wfd[j]) ok=0; }
      ...
      if(ok){ for(int k=0;k<K3;k++){ int o2=0; for(int j=0;j<3;j++){ for(int n=0;n<QN[j];n++) Wcat[(size_t)k*Ncat+o2+n]=Wq[j][(size_t)k*QN[j]+n]; o2+=QN[j]; } }
        wcat=ork_i8_mm_pack(c,K3,Ncat,Wcat); if(!wcat) ok=0; }
      ...
      for(size_t mi=0; ok && mi<sizeof BM/sizeof*BM; mi++){ int M=BM[mi];
        ...
        for(int j=0;j<3;j++) ork_i8_mm_run(c,wn[j],M,A,Cn[j]);
        long mism=0; for(int j=0;j<3;j++) for(size_t e=0;e<(size_t)M*QN[j];e++) if(Cf[j][e]!=Cn[j][e]){ mism++; break; }
        double t0=now_us(); for(int i=0;i<60;i++) for(int j=0;j<3;j++) ork_i8_mm_run(c,wn[j],M,A,Cn[j]); double us_3=(now_us()-t0)/60;
        double t1=now_us(); for(int i=0;i<60;i++) ork_i8_mm_run(c,wcat,M,A,Ccat); double us_c=(now_us()-t1)/60;
    ...
    { int K3=3584, GN=18944; int8_t *Wg[2], *A=malloc((size_t)128*K3); ork_w *wn[2],*wfd[2]; int ok=1;
      ...
      for(int j=0;j<2;j++){ Wg[j]=malloc((size_t)K3*GN); for(size_t i=0;i<(size_t)K3*GN;i++) Wg[j][i]=r8();
        wn[j]=ork_i8_mm_pack(c,K3,GN,Wg[j]);
        size_t fb=ork_i8_w_dump_fold_cpu(c,K3,GN,Wg[j],NULL,0); void*bl=malloc(fb); ork_i8_w_dump_fold_cpu(c,K3,GN,Wg[j],bl,fb);
        wfd[j]=ork_i8_mm_load_fold(c,K3,GN,bl,fb); free(bl); if(!wn[j]||!wfd[j]) ok=0; }
      ...
      for(size_t mi=0; ok && mi<sizeof BM/sizeof*BM; mi++){ int M=BM[mi];
        ...
        for(int j=0;j<2;j++) ork_i8_mm_run(c,wn[j],M,A,Cn[j]);
        long mism=0; for(int j=0;j<2;j++) for(size_t e=0;e<(size_t)M*GN;e++) if(Cf[j][e]!=Cn[j][e]){ mism++; break; }
        double t0=now_us(); for(int i=0;i<20;i++) for(int j=0;j<2;j++) ork_i8_mm_run(c,wn[j],M,A,Cn[j]); double us_n=(now_us()-t0)/20;
