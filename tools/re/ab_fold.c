/* ab_fold — #39 A/B: rkllm token-fold (ork_i8_npu_fold_run) vs ork-normal int8 matmul (ork_i8_mm_run), K=3584
 ...
 * fold us = avg submit (pure NPU); base us = full ork_i8_mm_run call. sudo env ORK_MM_TIMEOUT=800 ./ab_fold both 5 3
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
#include <time.h>
#include "ork_npu.h"
extern int ork_i8_npu_fold_run(ork_npu*,int,int,const int8_t*,int,const int8_t*,int32_t*,int,int,double*);
...
int main(int argc,char**argv){
    ...
    { int8_t*a8=calloc((size_t)8*K,1); int32_t*z=calloc((size_t)8*N,4); ork_i8_mm_run(c,ww,8,a8,z); free(a8); free(z); }  /* prewarm: activate domain + warm */
    ...
    for(int i=0;i<nM;i++){ int M=Ms[i]; double ub=0,uf=0; uint64_t hb=0,hf=0; int frc=0;
        if(dobase){ ork_i8_mm_run(c,ww,M,A,Cb); double t0=now_us(); for(int r=0;r<reps;r++) ork_i8_mm_run(c,ww,M,A,Cb); ub=(now_us()-t0)/reps; hb=fnv(Cb,(size_t)M*N); }
        if(dofold){ frc=ork_i8_npu_fold_run(c,K,N,W,M,A,Cf,ncore,reps,&uf); if(!frc) hf=fnv(Cf,(size_t)M*N); }
