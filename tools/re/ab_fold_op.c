/* ab_fold_op — #39 FULL-OP A/B: rkllm-style N-split fold (ork_npu_fold_op_i8) vs ork-normal (ork_mm_run_i8), for a
...
#include <time.h>
#include "ork_npu.h"
extern int ork_npu_fold_op_i8(ork_npu*,int,int,const int8_t*,int,const int8_t*,int32_t*,int,double*);
...
int main(int argc,char**argv){
    ...
    { int8_t*a8=calloc((size_t)8*K,1); int32_t*z=calloc((size_t)8*N,4); ork_mm_run_i8(c,ww,8,a8,z); free(a8); free(z); }  /* prewarm */
    ...
    for(int i=0;i<nM;i++){ int M=Ms[i]; double ub=0,uf=0;
        int frc=ork_npu_fold_op_i8(c,K,N,W,M,A,Cf,reps,&uf);
        if(frc){ printf("  %3d  fold rc=%d\n",M,frc); continue; }
        ork_mm_run_i8(c,ww,M,A,Cb); double t0=now_us(); for(int r=0;r<reps;r++) ork_mm_run_i8(c,ww,M,A,Cb); ub=(now_us()-t0)/reps;
