 ...
 * Runs rkllm's exact regcmd (/tmp/mm_regcmd.txt) via ork_i8_npu_replay but with OUR chosen A/W bytes:
...
int main(int argc,char**argv){
    ...
    { int8_t*wd=calloc(wsz,1); int32_t*td=calloc((size_t)8*N,4); int8_t*ad=calloc((size_t)8*K,1);   /* warm */
      ork_w*w=ork_i8_mm_pack(c,K,N,wd); if(w){ ork_i8_mm_run(c,w,8,ad,td); ork_mm_free(c,w);} free(wd);free(td);free(ad); }
    ...
    if(nmap){   /* ONE submit: weight[wposv[i]]=1<<i, A=1 -> C[m][n] = sum of bits for offsets mapping to (any k, n) */
        ...
        double us=0; int r=ork_i8_npu_replay(c,rc,rn,M,K,N,A,(int)asz,Wt,(int)wsz,Craw,1,&us);
