 ...
 *
 *   sudo tools/util/npu_guard.sh -- ./dom_scale_probe [iters] [M] [K] [N] [weights-per-domain]
 *
...
int main(int argc, char **argv){
    ...
    printf("dom_scale_probe: domains %d,%d  iters=%d  M=%d K=%d N=%d  nw=%d\n", d[0], d[1], iters, M, K, N, NW);
    ...
    printf("  packing %d resident weights (%d per domain, %.0f MiB total)\n", nres, NW, mib);
