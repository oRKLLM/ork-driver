 ...
 *   (a) 1-core            : ork_mm_run_i8, budget=1               (the working single-core R=32 matmul)
 *   (b) barrier 3-core    : ork_mm_run_i8, budget=3 (run_multicore N-split + barrier)  -- current prod
 *   (c) round-robin 3-core: split N into 3 slices, each a separate single-core matmul, dispatched
 *                           round-robin via ork_mm_run_stream_i8 (no barrier, 1 task/core)
...
int main(int argc,char**argv){
    ...
    ork_w*wf=ork_mm_pack_i8(c,K,N,B); if(!wf){printf("pack full failed\n");return 1;}
    ...
    for(int s=0;s<3;s++){
        ...
        ork_npu_set_pack_domain(c,0); ws[s]=ork_mm_pack_i8(c,K,Ns,Bs); if(!ws[s]){printf("pack slice %d failed\n",s);return 1;}
    ...
    /* (a) single-core */
    ork_npu_set_core_budget(c,1); ork_mm_run_i8(c,wf,M,A,C);
    if(do_check){ int mism=0; for(size_t i=0;i<(size_t)M*N;i++) if(C[i]!=Cref[i])mism++; printf("(a) 1-core    mism=%d\n",mism); }
    double t0=now_us(); for(int it=0;it<iters;it++) ork_mm_run_i8(c,wf,M,A,C); double t1=(now_us()-t0)/iters;

    /* (b) barrier 3-core */
    ork_npu_set_core_budget(c,cores); ork_mm_run_i8(c,wf,M,A,C);
    if(do_check){ int mism=0; for(size_t i=0;i<(size_t)M*N;i++) if(C[i]!=Cref[i])mism++; printf("(b) 3c-barrier mism=%d\n",mism); }
    t0=now_us(); for(int it=0;it<iters;it++) ork_mm_run_i8(c,wf,M,A,C); double tb=(now_us()-t0)/iters;
    ...
    t0=now_us(); for(int it=0;it<iters;it++) ork_mm_run_stream_i8(c,3,tk); double tc=(now_us()-t0)/iters;
