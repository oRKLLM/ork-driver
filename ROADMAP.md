...
5. **Robustness & Defensive Guards (Anti-Wedge Security)** — Prevent malformed inputs and invalid states from causing hardware-level deadlocks or IOMMU page faults that freeze `/dev/dri/cardN` and require physical SoC power cycles:
   ...
   * **Proposal IV: Cross-Process Arbitration via a Resource Ticket (Proposed)**:
     - *Rationale*: Guards I–III stop a *single* client's malformed job from wedging the NPU, but they cannot coordinate *across processes*. The NPU is single-stream; when two independent processes (e.g. the `orkllm` service and a stray `llama-bench`) submit concurrently they contend for the one engine, thrash each other's resident-weight cache and int4↔int8 mode state, and can stack into a wedge the kernel's per-submit queue won't untangle. A userspace *library* can't see another process's in-flight work, so this must live above the library.
     - *Mechanism (separation of concerns)*: an out-of-process **`ork-driver-orchestrator`** that does arbitration ONLY — it never touches tensors. Modeled on how cloud operators allocate GPUs: a client requests a **ticket** (lease) for NPU control, runs its burst of work, then **releases** the ticket; contending clients get a "red light" (queue or `-EBUSY`) until it frees. Compute stays fully in-process in `libork_npu`, so the zero-copy DMA hot path and per-op latency are untouched (no tensor marshaling, no per-matmul IPC). The library only acquires/holds/releases the ticket around a submit *burst* — transparently or via an explicit `ork_npu_lease_*` API; with no orchestrator present it behaves exactly as today (opt-in; `ORK_ORCH=0` to disable).
     - *Tiers*: (a) **MVP** — advisory `flock` on `/run/ork-driver.lock` (`LOCK_EX` = green light, `LOCK_NB` → `EWOULDBLOCK` = red); the kernel auto-releases the lock on process death, so crash-reclaim is free. No daemon, no fairness/priority, no visibility. (b) **Full** — a tiny C daemon on a unix socket with an `ACQUIRE(priority,id)` / `RELEASE` / `HEARTBEAT` ticket protocol: FIFO or priority fairness, socket-disconnect = implicit release (death detection), and a lease **TTL + heartbeat** so a holder that *hangs the hardware* is reclaimed — at which point the orchestrator issues `RKNPU_ACT_RESET` before granting the next ticket, centralizing the Proposal-III recovery.
     - *Scope/limits*: solves cross-process **contention** wedges and weight/mode thrash by serializing at session/burst granularity (one level above the kernel's per-submit queue). It does NOT prevent a green-lit client's malformed-job HW hang (that remains Proposals I–III) — but it bounds the blast radius from "manual power-cycle" to "automatic ticket reclaim + reset". Keeps ork-driver a library (C-only, no new runtime in the hot path); the daemon is the only added process (~a few hundred lines).
