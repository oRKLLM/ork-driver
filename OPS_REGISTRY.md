...
| resident int8 softmax seq assembly `[MATMUL_REQUANT_I8 → EXP_I8 → MM_I8]` (task #20) | compose the three proven pieces (K-pad int8 QK^T → scalar-max-biased exp → int8 Σ-reduce) into ONE `ork_submit_seq`, scores/e A2-resident | **PARTIAL — wedge solved via orkd; a reduce correctness bug remains** | `softmax_i8_seq_probe`. DIRECT: scores bit-exact (1 LSB) but wedges (int8-LUT-after-matmul trap → hang → `sudo reboot`). ORKD (N=64): **rc=0, NO wedge** — orkd's hardened path runs exp-after-matmul without hanging — but reduced Σ is garbage (direct Σ≈1, orkd Σ≈2e8 ⇒ the terminal MM_I8 reduce isn't writing its output under the resident chain). | **(B) orkd routing: the wedge is SOLVED** — `MATMUL_REQUANT_I8` routes through orkd (`reqi8_probe` ORKD K=512 → 1 LSB; the old "-3 TODO" was STALE, daemon-resident weight has `Bf` now), and the orkd-served seq does NOT hit the direct-mode exp-after-matmul wedge. **PARTIALLY diagnosed, NOT fully root-caused:** `softmax_i8_split_probe` (ORKD, N=64) split the chain — Stage A `[requant→exp]` returns `e` CORRECT (7 LSB, int8-LUT class; e[0] matches CPU) ⇒ **exp-after-matmul is fine (at N=64)**; Stage B standalone `[MM_I8 reduce]` at K=64 returns Σ GARBAGE (ss[0]=-67108106 ≈ -2²⁶). BUT the full 3-op chain at **N=512 (reduce K=512) ALSO fails** (Σ rel-err 1.4e4, rc=0), so **small-K is NOT the sole cause** — `int8reduce_probe` passing at K=512/M=64 doesn't generalize to the chain (M=512, A2-resident `e`, requant op before). Open variables: the A2 double-hop resident forwarding into the terminal doorbell reduce; M=512; whether the exp is still correct at M=512 (only validated at M=64). Needs **daemon-side instrumented debugging** (dump the reduce's actual A input + output on the daemon; re-validate `e` at N=512), NOT more black-box probe runs. **WEDGE WARNING: these int8-seq experiments wedged the NPU 4× (4 reboots) this session even on rc=0 runs — orkd avoids the exp-after-matmul HANG but the NPU still ends up wedged; investigate off the critical path.** LESSON: never leave orkd lingering between runs (stale daemon confound). | — |
